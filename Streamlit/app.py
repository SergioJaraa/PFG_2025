...
with col2:
    ...
    def load_stylegan_model_local(local_path):
        ...
        with open(local_path, "rb") as f:
            ...
            if "G_ema" in data:
                ...
                # fallback for model in raw state_dict
                G = networks.Generator(z_dim=512, c_dim=0, w_dim=512, img_resolution=512, img_channels=1)
    ...
    def generate_image(_G):
        _G = _G.to("cuda")
        with torch.no_grad():
            z = torch.randn([1, _G.z_dim], device="cuda")
            img = _G(z, None, truncation_psi=0.5, noise_mode='const')[0]
            img = (img.permute(1, 2, 0).detach().cpu().numpy() * 127.5 + 127.5).clip(0, 255).astype("uint8")
            return img
